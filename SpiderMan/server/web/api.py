...
from SpiderMan.model.model_to_dict import ModelsToDict
...
def scrapyd_object(host_id, ismodels=False, timeout=10):
    ...
    if ismodels is True:
        return ScrapydAPI_(
            base_url(
                host=host_info.host, port=host_info.port), timeout=timeout), host_info
    return ScrapydAPI_(
        base_url(
            host=host_info.host,
            port=host_info.port),
...
#             }, status_code=404)
#             return
#         ou_ocject = ModelsToDict(
#                 data=host_info,
#                 shield_field=[Host.scrapyd_name, Host.scrapyd_password], field_name={Host.id_:"id"}
#                 )
#         self.write({"data": ou_ocject})
...
class ListJobsHandler(BaseHandler):
    ...
    def get(self, host_id, project_name):
        ...
        try:
            self.write({"data": scrapyd.list_jobs(project_name)})
        except BaseException:
            self.write({"msg": "spider host error!",
                        "data": False}, status_code=500)
...
class ListSpiderHandler(BaseHandler):
    ...
    def get(self, host_id, project_name):
        ...
        scrapyd = scrapyd_object(host_id, timeout=1)
        try:
            self.write({"data": scrapyd.list_spiders(project_name)})
        except Exception as f:
            self.write({"data": False, "msg": str(f)}, status_code=500)
...
class ListProjectHandler(BaseHandler):
    ...
    def get(self, host_id):
        ...
        scrapyd = scrapyd_object(host_id, timeout=1)
        try:
            self.write({"data": scrapyd.list_projects()})
        except Exception as f:
            print(f)
            self.write({"msg": str(f), "data": False}, status_code=500)
            return
...
class StartProjectHandler(BaseHandler):
    ...
    def post(self):
        ...
        project_info = Project.getOne(
            Project.project_name == project_data['project_name'])
        if project_info:
            self.write({"data": False, "msg": "project name exist"},
                       status_code=404)
            return
    ...
    def delete(self, host_id, project_name):
        ...
        try:
            ...
            Timing.delete().where(
                ...
                Timing.project_name == project_name).execute()
            Project.update(
                host_id=None).where(
                Project.project_name == project_name,
                Project.host_id == host_id).execute()
        except Exception as f:
            self.write({"data": False, "msg": str(f)}, status_code=404)
            return []
        self.write({"data": True})
...
class UpdateFileHandler(BaseHandler):
    ...
    def post(self, project_id, file_name):
        project_info = Project.getOne(Project.id_ == project_id)
        if not project_info:
            self.write({"data": False}, status_code=404)
            return
        try:
            file_path = os.path.join(
                project_info.project_path,
    ...
    def get(self):
        ...
        try:
            for host_info in Host.select():
                data.append({'id': host_info.id_,
                             'host': host_info.host,
                             'port': host_info.port,
                             'name': host_info.name,
                             'create_time': time.strftime("%Y-%m-%d %H:%M:%S",
                                                          time.localtime(host_info.create_time)),
...
class ModifyConfHandler(BaseHandler):
    ...
    def post(self, host_id):
        ...
        if not is_run:
            try:
                requests.get("http://{}:{}".format(self.get_argument('host'),
                                                   self.get_argument('port')), timeout=0.1)
                is_run = True
            except Exception as f:
                print(f)
        up_item = {
    ...
    def get(self, host_id):
        ...
        try:
            host_info = Host.getOne(Host.id_ == host_id)
            self.write({"data": {'host': host_info.host,
                                 'name': host_info.name,
                                 'scrapyd_name': host_info.scrapyd_name,
                                 'scrapyd_password': host_info.scrapyd_password,
                                 'host_ssh_name': host_info.host_ssh_name,
                                 'host_ssh_password': host_info.host_ssh_password,
                                 'port': host_info.port}})
        except BaseException:
            self.write({"data": False, "msg": "host id not find"},
                       status_code=404)
...
class SetTimingHandler(BaseHandler):
    ...
    def post(self, host_id, project_name, spider_name, _time):
        try:
            timing_info = Timing.getOne(
                Timing.host_id == host_id,
                Timing.project_name == project_name,
            ...
            if not timing_info:
                ...
                # 更新
                Timing.update(last_time=time.time(), run_time=_time).execute()
            self.write({"data": True})
        except Exception as f:
...
class GetTimingHandler(BaseHandler):
    ...
    def get(self, host_id, project_name, spider_name):
        ...
        if not timing_info:
            self.write({"data": False,
                        'msg': "not find project and not find spider"})
        else:
            out_object = ModelsToDict(
                data=timing_info, field_name={
                    Timing.id_: "id"})
...
class HostSshHandler(BaseHandler):
    ...
    def get(self, host_id):
        ...
        try:
            out_object = ModelsToDict(
                data=host_info,
                shield_field=[Host.scrapyd_password, Host.scrapyd_name],
                field_handle={Host.host: getIp},
                field_name={Host.id_: "id"}
            )
            self.write({"data": out_object})
        except Exception as f:
            self.write(
                {"data": False, "msg": str(f)})
            self.set_status(status_code=404)
...
class LocationListProjectHandler(BaseHandler):
    ...
    def get(self, *args, **kwargs):
        ...
        def time2time(x): return time.strftime(
            "%Y-%m-%d %H:%M:%S", time.localtime(x))
        out_object = ModelsToDict(
            data=Project.select(), field_name={
                Project.id_: "id"}, field_handle={
                Project.create_time: time2time, Project.update_time: time2time})
...
class NewHostHandler(BaseHandler):
    ...
    def post(self, *args, **kwargs):
        try:
            isrun = True
            try:
                requests.get("http://{}:{}".format(self.get_argument('host_id'),
                                                   self.get_argument('host_port')), timeout=0.1)
            except BaseException:
                isrun = False
            ...
            Host.create(
                host=self.get_argument('host_id'),
                name=self.get_argument('host_name'),
                scrapyd_name=self.get_argument(
                    'scrapy_name',
                    ''),
                scrapyd_password=self.get_argument(
                    'scrapy_password',
                    ''),
                host_ssh_name=self.get_argument(
                    'ssh_name',
                    ''),
                host_ssh_password=self.get_argument('ssh_password'),
...
class DeleteLocationProjectHandler(BaseHandler):
    ...
    def delete(self, project_name):
        ...
        if not project_info:
            self.write({"data": False,
                        "msg": "{} project not sxist!".format(project_name)})
            self.set_status(status_code=404)
            return
        try:

            if not delete_scrapy_project(
...
class DeployProjectHandler(BaseHandler):
    ...
    def post(self, host_id, project_id):
        ...
        scrapyd = scrapyd_object(host_id=host_id)
        try:
            scrapyd.add_version(project=project_info.project_name, egg=open(
                egg_file_path, 'rb').read(), version=project_info.project_version)
            Project.update(
                host_id=host_id).where(
                Project.id_ == project_id).execute()
            self.write({"data": True})
        except Exception as f:
            import traceback
            traceback.print_exc()
